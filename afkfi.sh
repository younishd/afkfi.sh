...
pactl list short sinks | grep -q afkfish || (pactl load-module module-combine-sink slaves="$SINK" sink_name=afkfish sink_properties=device.description="afkfi.sh" >/dev/null)

pactl move-sink-input "$SINKINPUT" afkfish
